...
The system must also respect memory scopes—user, session, agent and application—and design them early, because changing scoping later is costly [].  A concurrency control layer must prevent stale writes and coordinate updates.

**Offline vs online RL:** Online RL methods like PPO require on‑the‑fly data collection and are expensive; offline methods such as **OREO** leverage logged trajectories, learn a policy and value function via the soft Bellman equation, and require no paired preference data [].  However, pure offline methods may overfit to logged behaviour and fail to explore; a hybrid approach with occasional online rollouts is planned.
...
**A2A delegation:** Hand‑offs must be explicit.  Each agent advertises capabilities, accepts sub‑tasks with clear contracts (inputs, expected outputs and error codes) and returns control to the delegating agent upon completion.  Delegation should involve shared memory segments rather than full conversation histories to avoid “context dumping” and token bloat[].
...
* **Shared ticket memory** acts as the central store for the ticket state, accessible by all agents involved in the session.  It ensures that agents operate on the same reality and reduces duplication and inconsistent responses [].  Access control lists and versioning prevent race conditions.
...
3. **Offline RL training** – use algorithms like **OREO** which jointly learn a policy and value function using the soft Bellman equation and can work from unpaired trajectories[].  OREO avoids the need for pairwise preference data and provides better credit assignment in multi‑step reasoning[].  
...
* **Reasoning metrics:** plan quality, plan adherence, tool selection accuracy and deviation from plan[].  
* **Action metrics:** tool correctness, argument correctness and execution path validity[].  
* **End‑to‑end metrics:** task completion, step efficiency, latency and cost[].  
* **Safety metrics:** prompt injection resilience, policy adherence and bias detection [].

The harness uses **tracing** to capture every decision, tool call and response [].  Each commit triggers the harness on a suite of scenario‑based tests drawn from real tickets; regression gates block deploys when metrics drop.

### 7. Guardrails and Supervisory Layer

Security and safety controls are layered:

1. **Black‑box guardrails** wrap around the agent and implement input and output filtering.  Input guardrails block harmful prompts, detect PII, filter sensitive topics and prevent prompt injection [].  Output guardrails filter responses for harmful, biased or factually incorrect content [].
2. **Guardrails by design** embed security into the architecture.  Secure workflows incorporate threat modelling and proper authentication and authorisation [].  The **principle of least privilege** grants agents only the permissions they need[].  **Security‑first prompting** and **integrated monitoring** with audit trails and real‑time threat detection provide ongoing visibility [].  
3. **Governance guardrails** ensure agents optimise for intended outcomes rather than proxy metrics.  Input guardrails detect prompt injection that could change the agent’s goal, and tool guardrails require human approval for goal or policy changes [].  Version control for prompts and models plus behavioural drift monitoring mitigate policy drift[].
...
* **Hybrid memory and concurrency control** – ensures consistency across agents and avoids token bloat while scaling to thousands of sessions.  Scoping dimensions (user, session, agent, application) ensure agents see only relevant information [].
...
* **RL training pipeline** – offline RL (OREO) is chosen to leverage logged data and reduce the cost of online interaction [].  Reward shaping uses real business metrics to align agent behaviour with customer satisfaction while avoiding reward hacking.
* **Continuous evaluation and guardrails** – evaluation harness integrated into CI ensures regressions are caught early.  Layered guardrails provide both fast filtering (black‑box) and deep safety controls (by design) [], [].
